...
function onDropShadowChange(newValue) {
  if (newValue == 'true')
    textToType.style.textShadow = "5px 5px 1px #ff0000"
  else
    textToType.style.textShadow = 'none'
}
shadowFolder.add(outerTextToTypeStyle, 'dropShadow', {Yes: true, No: false}).onChange(onDropShadowChange).name('Drop shadow')
...
backgroundStyleFolder.addColor(document.body.style, 'backgroundColor').onChange((newColor) => textToType.style.backgroundColor = newColor).name('Background color')
